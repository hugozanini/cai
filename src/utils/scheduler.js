...
export async function runScheduler(token, preferences) {
    ...
    try {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            if (!hasLunch && preferences.lunchDuration > 0) {
                ...
                if (lunchSlotDetails) {
                    ...
                    createPromises.push(createEvent(token, {
                        summary: '🍽️ Lunch',
        ...
        if (createPromises.length > 0) {
            console.log(`Executing ${createPromises.length} create event requests concurrently...`);
    ...
    try {
        ...
        for (const ev of rawEvents) {
            ...
            if (isCaiEvent) {
                ...
                deletePromises.push(deleteEvent(token, ev.id));
            }
        }

        if (deletePromises.length > 0) {
            console.log(`Executing ${deletePromises.length} delete event requests concurrently...`);
            await Promise.all(deletePromises);
        }

        console.log(`Successfully cleared ${deletePromises.length} Cai events.`);
