...
export async function runScheduler(token, preferences) {
    ...
    try {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            if (!hasLunch && preferences.lunchDuration > 0) {
                ...
                if (lunchSlotDetails) {
                    ...
                    createPayloads.push({
                        summary: '🍽️ Lunch',
                        description: '<i>Generated by <b>Cai</b> – your calendar intelligence.</i><br/><br/>Enjoy your meal and take a well-deserved break!',
            ...
            if (!hasCoffee && coffeeMins > 0) {
                ...
                if (coffeeSlotDetails) {
                    ...
                    createPayloads.push({
                        ...
                        description: '<i>Generated by <b>Cai</b> – your calendar intelligence.</i><br/><br/>Take a moment to step away and recharge.',
            ...
            if (neededSecs > 0) {
                ...
                for (const slot of freeSlots) {
                    ...
                    while (slotDurationSecs >= 1800 && neededSecs > 0) { // at least 30 mins
                        ...
                        createPayloads.push({
                            summary: '💡 Focus Time',
                            description: '<i>Generated by <b>Cai</b> – your calendar intelligence.</i><br/><br/>This block is protected for deep work. Silence your notifications and focus on what matters most.',
        ...
        if (createPayloads.length > 0) {
            console.log(`Executing ${createPayloads.length} create event requests incrementally...`);
            await executeInBatches(createPayloads, (payload) => createEvent(token, payload), 5, 400);
        }
...
export async function clearAllCaiEvents(token) {
    ...
    try {
        ...
        for (const ev of rawEvents) {
            ...
            if (isCaiEvent) {
                console.log(`Queueing deletion for Cai event: ${ev.summary} on ${startDate}`);
                deleteIds.push(ev.id);
            }
        }

        if (deleteIds.length > 0) {
            console.log(`Executing ${deleteIds.length} delete event requests incrementally...`);
            await executeInBatches(deleteIds, (id) => deleteEvent(token, id), 5, 400);
        }

        console.log(`Successfully cleared ${deleteIds.length} Cai events.`);
